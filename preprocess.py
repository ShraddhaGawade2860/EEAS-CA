...
def get_input_data() -> pd.DataFrame:
    ...
    # Rename type columns for consistency
    rename_map = {'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}
    ...
    # Ensure text columns are unicode strings
    ...
    # Add a 'y' column for main class for convenience
    df["y"] = df[Config.CLASS_COL]

    # Filter rows where main class label exists and is not empty
...
def de_duplication(data: pd.DataFrame) -> pd.DataFrame:
    ...
    cu_pattern = '|'.join([f"({expr})" for lang in cu_template for expr in cu_template[lang]])
